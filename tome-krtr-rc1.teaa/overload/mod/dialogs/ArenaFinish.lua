...
function _M:printRanking()
	...
	if not scores[1].name then return "#LIGHT_GREEN#고득점 기록이 없습니다. 이런일이 있을순 없는데요."
		...
		while(scores[i] and scores[i].name) do
			p = scores[i]
			tmp = stri:format((p.name or "unknown"):capitalize(), p.sex:krSex() or "unknown", p.race:krRace() or "unknown", p.class:krClass() or "unknown", p.score or "unknown", p.perk or "unknown", p.wave or -1)
