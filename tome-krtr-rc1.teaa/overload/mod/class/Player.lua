...
function _M:restCheck()
	...
	if #spotted > 0 then
		...
		return false, ("%s의 적대적 존재 발견 (%s%s)"):format(dir, san, game.level.map:isOnScreen(spotted[1].x, spotted[1].y) and "" or " - 화면바깥")
	end
...
function _M:hotkeyInventory(name)
	...
	if not o then
		Dialog:simpleLongPopup("아이템 없음", "당신은 "..name:addJosa("를").." 찾지 못했습니다.", game.w * 0.4)
	else
		-- Wear it ??
