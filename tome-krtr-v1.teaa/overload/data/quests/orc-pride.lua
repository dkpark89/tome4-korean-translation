...
kr_display_name = "수많은 오크의 무리들"
desc = function(self, who)
	local desc = {}
	desc[#desc+1] = "오크 무리들이 있는 곳을 조사해야 합니다."

	if self:isCompleted("rak-shor") then
		desc[#desc+1] = "#LIGHT_GREEN#* 당신은 락'쇼르를 처치했습니다.#WHITE#"
	else
		desc[#desc+1] = "#SLATE#* 락'쇼르 무리, 남쪽에 있는 사막의 서쪽 지역#WHITE#"
	end
--[[
	...
	if self:isCompleted("vor") then
		...
		desc[#desc+1] = "#SLATE#* 보르 무리, 북동쪽.#WHITE#"
	end
	...
	if self:isCompleted() then
		desc[#desc+1] = ""
		desc[#desc+1] = "#LIGHT_GREEN#* 모든 오크 무리들은 파괴되고, 그들의 지도자는 죽음을 맞이했습니다. 고위 태양의 기사 아에린이 이 소식을 들으면 굉장히 기뻐할 것입니다!#WHITE#"
	end
