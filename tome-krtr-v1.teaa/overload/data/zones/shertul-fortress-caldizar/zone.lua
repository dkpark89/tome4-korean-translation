...
return {
	...
	on_enter = function(lev, old_lev, zone)
		...
		Dialog:simpleLongPopup("미지의 쉐르'툴 요새", "갑작스런 충격과 함께, 당신은 어쩐지 익숙한 곳에 있음을 발견했습니다... 부드러운 벽과 온화한 빛이 당신의 요새를 떠올리게 만듭니다. 하지만 뭔가가 아직 다르다고 느껴지기도 합니다. 정다운 웅웅거리는 소리가 어디선가 들려오고, 신체가 거의 무게가 없는 것처럼 가벼워지는 것을 느끼며, 작은 움직임만으로도 허공으로 나아가게 됩니다. 당신이 더이상 마즈'에이알에 있지 않다는 이상한 느낌이 듭니다... 앞쪽에 끔찍하고 환상적인 존재가 있음이 동시에 느껴지고, 당신의 모든 말단이 떨려옴을 느낍니다.", 500)
	end,
