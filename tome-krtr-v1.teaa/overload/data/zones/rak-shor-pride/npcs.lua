...
newEntity{ base = "BASE_NPC_GHOUL", define_as = "ROTTING_TITAN",
	...
	kr_display_name = "썩은 타이탄",
	resolvers.nice_tile{image="invis.png", add_mos = {{image="npc/undead_ghoul_rotting_titan.png", display_h=2, display_y=-1}}},
	desc = [[이 거대한 살점과 암석이 섞인 덩어리는 느리게 움직이고 있고, 그가 걸음을 옮길 때마다 대지가 울려 퍼집니다. 그 몸은 끊임없이 진동하면서 개량되고 있는 것 같습니다. 수족 끝에 달린 대형 암석은 대형 둔기와 같은 효과를 낼 것 같습니다.]],
	...
	on_move = function(self)
			if rng.percent(20) then
				game.logSeen(self, "%s의 걸음으로 대지가 흔들립니다!", (self.kr_display_name or self.name):capitalize())
...
newEntity{ base = "BASE_NPC_BONE_GIANT", define_as = "HEAVY_SENTINEL",
	...
	kr_display_name = "무거운 파수꾼",
	desc = [[셀수 없을 많큼 많은 존재들의 뼈로 만들어진 아주 키가 큰 생명체입니다. 흉부 안쪽에서부터 불꽃의 오러가 명렬히 피어오르고 있습니다.]],
