...
newTalent{
	...
	attack = function(self, t, target)
		...
		if self:checkHit(mindpower, target:combatMentalResist()) then
			...
			game.logSeen(self, "%s 고통의 손짓을 저항했습니다.", (target.kr_display_name or target.name):capitalize():addJosa("가"))
	...
	info = function(self, t)
		...
		최소한 한 손은 비어있거나 마석만을 들고 있어야 손짓을 취할 수 있으며, 피해량은 정신력 능력치의 영향을 받아 증가합니다.
...
newTalent{
	...
	info = function(self, t)
		...
		최소한 한 손은 비어있거나 마석만을 들고 있어야 손짓을 취할 수 있으며, 이 기술은 고통의 손짓을 유지하지 않아도 적용됩니다.]]):format(mindpowerChange, mindCritChange, self:combatMindCrit())
	end,
}

newTalent{
	name = "Gesture of Guarding",
	kr_display_name = "수호의 손짓",
	...
	on_hit = function(self, t, who)
		if rng.percent(t.getCounterAttackChance(self, t)) and self:isTalentActive(self.T_GESTURE_OF_PAIN) and canUseGestures(self) then
			game.logSeen(self, "#F53CBE#%s %s의 공격을 반격합니다!", (self.kr_display_name or self.name):capitalize():addJosa("가"), (who.kr_display_name or who.name))
	...
	info = function(self, t)
		...
		최소한 한 손은 비어있거나 마석만을 들고 있어야 손짓을 취할 수 있으며, 이 기술은 고통의 손짓을 유지하지 않아도 적용됩니다.]]):format(-damageChange, counterAttackChance)
	end,
}
