...
local function how_speak(o)
	...
	local sentient_responses = {
		default = [[아, 나는 이전에 두려울 정도로 전지전능한 영매였지. 전지전능했었지만 그래, 멍청했었어. 결국 이렇게 되어버렸지. 약간의 사고로 인해 주입 기술 '쿠갈라의 영혼 역행' 에 걸려버렸어. 내 상황을 요약하자면, 내 영혼은 이제 이 막대기 속에 갇혀버렸어, 그리고 내가 같이 일하던 영혼은... 뭐, 그가 어디로 가버렸는지는 자세히는 모르지만. 그러나 난 우리가 그를 만나는 일은 없었으면 하고 바라고 있어.]],
