...
function _M:generateList()
	...
	list[#list+1] = { zone=zone, name=string.toTString"#GOLD##{bold}#FPS 설정#WHITE##{normal}#", status=function(item)
		...
		game:registerDialog(GetQuantity.new("재생 빈도를 입력하시오", "5에서 60까지", config.settings.display_fps, 60, function(qty)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=string.toTString"입자 효과(particle effects)의 밀도를 결정합니다.\n이 설정은 게임 내부의 여러가지 입자 효과들이 얼마나 자세하게 표현될지를 결정합니다.\n주문 효과 등의 표현시 게임이 너무 느려진다면 이 설정치를 낮추세요.#WHITE#"}
	list[#list+1] = { zone=zone, name=string.toTString"#GOLD##{bold}#입자 효과 밀도#WHITE##{normal}#", status=function(item)
		...
		game:registerDialog(GetQuantity.new("밀도를 입력하시오", "0부터 100까지", config.settings.particles_density, 100, function(qty)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=string.toTString"글자들을 매끄럽게 보여줄지(antialiasing) 결정합니다.\n글자들이 보기좋아 지지만, 게임속도가 조금 느려질 수 있습니다.\n\n#LIGHT_RED#이 설정의 효과는 게임을 다시 시작해야 적용됩니다.#WHITE#"}
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=string.toTString"OpenGL의 그림자효과(OpenGL Shaders)를 사용할지 결정합니다.\n이 설정을 사용하면 몇몇 특별한 화면 효과를 볼 수 있습니다.\n만약 화면이 이상하게 갑작스런 변화(glitch)가 발생하면 이 설정을 사용안함으로 바꿔보십시오.\n\n#LIGHT_RED#이 설정의 효과는 게임을 다시 시작해야 적용됩니다.#WHITE#"}
	...
	list[#list+1] = { zone=zone, name=string.toTString"#GOLD##{bold}#감마 보정값 설정#WHITE##{normal}#", status=function(item)
		...
		game:registerDialog(GetQuantity.new("감마 보정값 설정", "50부터 300까지", config.settings.gamma_correction, 300, function(qty)
