...
function _M:getName(t)
	...
	local name = self.kr_display_name ~= nil and type(self.kr_display_name) == "string" and self.kr_display_name or self.name
	...
	if req.talent then
		for _, tid in ipairs(req.talent) do
			if type(tid) == "table" then
				--@@
				local tn = who:getTalentFromId(tid[1]).kr_display_name
				...
				--@@
				local tn = who:getTalentFromId(tid).kr_display_name
