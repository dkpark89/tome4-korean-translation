...
newEntity{ define_as = "ATAMATHON", base = "BASE_NPC_CONSTRUCT",
	...
	desc = [[이 거인 골렘은 장작더미의 시대에 하플링이 오크에 대항하기 위한 무기로 만들었지만 포식자 가르쿨에게 파괴되었다고 알려져 있습니다. 어떤 어리석은 이가 다시 재건하려 했지만 그 제어를 할 수 없었고, 이제 이것은 오래전에 죽은 원제작자에 대해 격노한 상태입니다. 대리석으로 만들어진 몸에 단단한 보라툰으로 관절부위가 연결되고, 가장 순수한 루비가 눈으로 사용되었습니다.. 12미터가 넘는 높이에서 격노에 차 빛나는 핏빛 구슬이 보입니다.]],
