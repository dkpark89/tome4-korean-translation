	...
	action = function(self, t)
		...
		for i = start, start + 8 do
			...
			if target and not target.dead and self:reactionToward(target) < 0 then
				game.logSeen(self, "#F53CBE#%s %s 후려칩니다!", (self.kr_display_name or self.name):capitalize():addJosa("가"), (target.kr_display_name or target.name):addJosa("를"))
				...
				if target:canBe("stun") then
					...
					game.logSeen(target, "#F53CBE#%s 기절하지 않았습니다!", (target.kr_display_name or target.name):capitalize():addJosa("가"))
				end
