...
newTalent{
	...
	action = function(self, t)
		...
		return ([[눈 깜짝할 사이에 최대 %d 칸 떨어진 대상의 옆에 나타나, %d%% 피해를 줍니다.]]):format(self:getTalentRange(t), multiplier)
	end,
}
	...
	action = function(self, t)
		...
		if self:reactionToward(target) < 0 then
			...
			if shadowCount > 0 then
				game.logPlayer(self, "#PINK#그림자들이 %s에게 모여듭니다!", (target.kr_display_name or target.name))
				return true
			else
