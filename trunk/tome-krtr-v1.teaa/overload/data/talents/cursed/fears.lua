...
newTalent{
	...
	applyEffect = function(self, t, target)
		if not target:canBe("fear") then
			game.logSeen(target, "#F53CBE#%s 공포를 무시합니다!", (target.kr_display_name or target.name):capitalize():addJosa("가"))
			return true
		end
		...
		if not target:checkHit(mindpower, target:combatMentalResist()) then
			game.logSeen(target, "%s 공포를 이겨냈습니다!", (target.kr_display_name or target.name):capitalize():addJosa("가"))
			return nil
		end
...
newTalent{
	...
	info = function(self, t)
		...
		그리고, 두 가지 새로운 공포 효과를 사용할 수 있게 됩니다.
		- 겁에 질림 : 다른 공포에 걸릴 때마다, 겁에 질려 %d 정신 피해를 받습니다. 이전에 걸려있던 공포에도 적용됩니다.
		- 고통 : %d 마리의 '고통주는 자' 가 나타나 대상을 공격합니다. 이들은 사라지기 전까지 %d 정신 피해를 줍니다.
			...
			function(px, py)
				...
				if actor and self:reactionToward(actor) < 0 and actor ~= self then
					if not actor:canBe("fear") then
						game.logSeen(actor, "#F53CBE#%s 공황 상태를 무시합니다!", (actor.kr_display_name or actor.name):capitalize():addJosa("가"))
					elseif actor:checkHit(self:combatMindpower(), actor:combatMentalResist(), 0, 95) then
						...
						game.logSeen(actor, "#F53CBE#%s 공황 상태를 저항합니다!", (actor.kr_display_name or actor.name):capitalize():addJosa("가"))
					end
				end
			end,
			nil, nil)
