...
local function forceHit(self, target, sourceX, sourceY, damage, knockback, knockbackDamage, power, max)
	...
	if not target.dead and knockback and knockback > 0 and target:canBe("knockback") and (target.never_move or 0) < 1 then
			...
			if not game.level.map:isBound(x, y) or is_corner_blocked or game.level.map:checkAllEntities(x, y, "block_move", target) then
				...
				if nextTarget then
					if knockbackCount > 0 then
						game.logPlayer(self, "%s %d 칸 밀려나 %s 부딪쳤습니다!", (target.kr_display_name or target.name):capitalize():addJosa("은"), knockbackCount, (nextTarget.kr_display_name or nextTarget.name):addJosa("와"))
					else
						game.logPlayer(self, "%s %s 부딪쳤습니다!", (target.kr_display_name or target.name):capitalize():addJosa("은"), (nextTarget.kr_display_name or nextTarget.name):addJosa("와"))
					end
				elseif knockbackCount > 0 then
					game.logPlayer(self, "%s %d 칸 밀려났습니다!", (target.kr_display_name or target.name):capitalize():addJosa("은"), knockbackCount)
				else
					game.logPlayer(self, "%s 밀려났습니다!", (target.kr_display_name or target.name):capitalize():addJosa("은"))
				end
		...
		if not blocked and knockbackCount > 0 then
			game.logPlayer(self, "%s %d 칸 밀려났습니다!", (target.kr_display_name or target.name):capitalize():addJosa("은"))
		end
...
newTalent{
	...
	action = function(self, t)
		game.logSeen(self, "보이지 않는 힘이 %s 주변을 휘몰아치기 시작합니다!", (self.kr_display_name or self.name))
	...
	do_unseenForce = function(self, t)
		...
		if self.unseenForce.duration <= 0 then
			...
			game.logSeen(self, "%s 주변을 휘몰아치던 보이지 않는 힘이 사라집니다.", (self.kr_display_name or self.name))
		end
	end,
	on_learn = function(self, t)
		self.combat_critical_power = (self.combat_critical_power or 0) + 3
