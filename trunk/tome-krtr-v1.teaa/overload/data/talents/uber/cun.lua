...
uberTalent{
	...
	info = function(self, t)
		...
		공격이 냉기 속성이었을 경우, 30%% 확률로 5 턴 동안 피부가 얼음이 되어 적에게 100 냉기 피해를 돌려주고 물리 피해를 30%% 덜 받게 됩니다.
		공격이 전기 속성이었을 경우, 30%% 확률로 3 턴 동안 순수한 전격의 존재가 되어 받은 피해를 무효화하고 근처로 순간이동합니다. (한 턴에 한 번만 가능합니다)
		공격이 빛 속성이었을 경우, 30%% 확률로 보호막을 만들어 5 턴 동안 200 피해를 흡수합니다.
		공격이 자연 속성이었을 경우, 30%% 확률로 신체가 강화되어 5 턴 동안 해로운 마법적 상태효과에 걸리지 않게 됩니다.
		모든 효과는 200 이상의 피해를 줬을 경우에만 발동합니다.]])
		:format()
	end,
}
...
uberTalent{
	...
	kr_display_name = "세상의 모든 지식",
	mode = "passive",
	on_learn = function(self, t, kind)
		local Chat = require "engine.Chat"
		local chat = Chat.new("worldly-knowledge", {name="Wordly Knowledge", kr_display_name="세상의 모든 지식"}, self)
		chat:invoke()
	end,
	info = function(self, t)
