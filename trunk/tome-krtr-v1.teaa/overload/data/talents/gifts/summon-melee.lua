		...
		local m = NPC.new{
			...
			on_takehit = function(self, value, src)
				...
				if self.summoner and not self.summoner.dead then
					...
					game.logSeen(self, "#GREEN#%s 충격의 일부를 흡수했습니다. %s 자연과 조금 더 가까워졌습니다.", (self.kr_display_name or self.name):capitalize():addJosa("가"), (self.summoner.kr_display_name or self.summoner.name):capitalize():addJosa("는"))
				end
				return value - p
			end,
		}
