...
그는 이곳 태양의 장벽에 사는 마법사로, 괴짜긴 하지만 숙련된 마법사죠. 그리고 마즈'에이알로 가는 새로운 장거리 관문을 만들 수 있다고 생각하는 사람이기도 하죠.
...
그러니... 제메키스를 만나보는 것이 좋을 것 같습니다. 당신이 가지고 있는 여러 장소의 오브를 보면 분명 그는 엄청난 관심을 보일테니까요. 그는 바로 북쪽에 있는 작은 집에서 살고 있습니다.]],
	answers = {
...
그들의 지도자들이 죽었다? 그것도 단 한명의 @playerdescriptor.race@ 손에? 정말 놀라울 정도로 강하군요, 당신은.
...
그리고 그는 최고봉으로 가는 길이 "슬라임 터널" 에 있다고 했습니다. 오크 서식지 중 어딘가, 아마 그루쉬낙 무리들이 있던 곳의 어딘가에 있다는군요.
]],
	answers = {
		{"감사합니다. 슬라임 터널을 찾아, 최고봉으로 가보겠습니다.", action=function(npc, player)
