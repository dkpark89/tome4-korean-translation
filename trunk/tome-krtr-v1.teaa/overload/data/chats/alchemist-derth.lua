...
local function alchemist_reaction_complete(npc, player, lose, other_alch, other_elixir)
	if lose == true then
		...
		return ([[아, 좋군 그래. 이리 주게나. 하지만 자네가 오랫동안 자리를 비운 동안, %s 놈이 %s를 만들었다는 소식을 들었다네. 그놈이 내 자리를 뺏는걸 볼 바에야, 차라리 내 목숨을 끊고 말테니 그리 알게.]]):format(other_alch, other_elixir)
	end
end
