	...
	if _M.tempeffect_def[eff_id].on_gain then
		...
		if not silent and not had then
			if ret then
				...
				game.logSeen(self, ret:gsub("#Target#", sn:capitalize()):gsub("#target#", sn):gsub("#Target1#", sn:capitalize():addJosa("가")):gsub("#target1#", sn:addJosa("가")):gsub("#Target2#", sn:capitalize():addJosa("는")):gsub("#target2#", sn:addJosa("는")):gsub("#Target3#", sn:capitalize():addJosa("를")):gsub("#target3#", sn:addJosa("를")):gsub("#Target4#", sn:capitalize():addJosa("로")):gsub("#target4#", sn:addJosa("로")):gsub("#Target5#", sn:capitalize():addJosa("다")):gsub("#target5#", sn:addJosa("다")):gsub("#Target6#", sn:capitalize():addJosa("과")):gsub("#target6#", sn:addJosa("과"))) --@@ 조사 추가 코드 삽입 
			end
