	...
	if not t.generic then
		if v then
			if self.actor.unused_talents < 1 then
				self:simpleLongPopup("직업기술 점수 부족", "남아있는 직업기술 점수가 없습니다!", 300)
				return
			end
			if not self.actor:canLearnTalent(t) then
				self:simpleLongPopup("기술을 배울수 없음", "선행조건에 부합하지 않아 배울 수 없습니다!", 300)
				return
			end
		...
		if v then
			if self.actor.unused_generics < 1 then
				self:simpleLongPopup("일반기술 점수 부족", "남아있는 일반기술 점수가 없습니다!", 300)
				return
			end
			if not self.actor:canLearnTalent(t) then
				self:simpleLongPopup("기술을 배울수 없음", "선행조건에 부합하지 않아 배울 수 없습니다!", 300)
