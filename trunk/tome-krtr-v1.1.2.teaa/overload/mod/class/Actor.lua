...
function _M:tooltip(x, y, seen_by)
	...
	for tid, act in pairs(self.sustain_talents) do
		if act then ts:add(true, "- ", {"color", "LIGHT_GREEN"}, self:getTalentFromId(tid) and (self:getTalentFromId(tid).kr_name or self:getTalentFromId(tid).name) or "???", {"color", "WHITE"} ) end
	end
...
function _M:preUseTalent(ab, silent, fake)
	--@ 다음줄, 두줄뒤 - 세줄뒤부터 이 함수 끝(232여줄뒤, 현재 4037)까지 사용 : 반복된 이용으로 변수로 뺌
...
function _M:getTalentFullDescription(t, addlevel, config, fake_mastery)
	...
	if not config.ignore_ressources then
		...
		if t.stamina then d:add({"color",0x6f,0xff,0x83}, "체력 소모량 : ", {"color",0xff,0xcc,0x80}, ""..math.round(util.getval(t.stamina, self, t) * (100 + self:combatFatigue()) / 100, 0.1), true) end
	...
	if t.mode ~= "passive" then
		if self:getTalentRange(t) > 1 then d:add({"color",0x6f,0xff,0x83}, "최대 사거리 : ", {"color",0xFF,0xFF,0xFF}, ("%0.1f"):format(self:getTalentRange(t)), true)
