...
function _M:handleEffect(player, eff_id, e, p, x, y, hs, bx, by, is_first, scale, allow_remove)
	...
	if not self.tbuff[eff_id..":"..dur..":"..charges] then
		...
		if p.save_string and p.amount_decreased and p.maximum and p.total_dur then
			desc = ("#{bold}##GOLD#%s\n(%s: %s)#WHITE##{normal}#\n"):format(name, e.type:krEffectType(), eff_subtype)..e.long_desc(player, p).." "..("%s 이 효과의 지속시간을 %d 턴에서 %d 턴으로 %d턴 만큼 줄였습니다."):format(p.save_string:addJosa("가"), p.maximum, p.total_dur, p.amount_decreased) --@ 변수 순서 조정, 임시 효과 분류 한글로 바꿈
		else
