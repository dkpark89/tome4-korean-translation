...
newChat{ id="welcome",
	text = [[#LIGHT_GREEN#*아무도 응답하지 않습니다.*#WHITE#]],
	answers = {
		{"[떠난다]"},
	}
}
...
newChat{ id="east_portal4",
	text = [[만약 오크들이 레크놀 깊은 곳에 관문을 만들었다면, 그 물건들 역시 어떻게든 사용을 했겠지. 그리고 제례단검과 다이아몬드가 그들이 만들어낸 관문을 넘어오지 못한다면, 그것들 역시 이곳 마즈'에이알 대륙에 존재하지 않을까? 레크놀에 있는 관문 근처를 우선 조사해보는 것이 좋을 것 같네. 그들이 관문을 만들고 제례단검과 다이아몬드를 옮기지 않았을 수도 있으니 말이야.]],
	answers = {
...
newChat{ id="has_material_gave_orb",
	text = [[아주 좋네. 자네가 그것들을 찾는 동안, 나는 모든 준비를 마쳤다네. 오, 이것을 받게. #LIGHT_GREEN#*그가 열쇠 하나를 건네줍니다.*#WHITE# 이 열쇠는 텔무르의 폐허를 여는데 쓰인다네. 오랜 세월 동안 봉인된 숄타르 인이 있는 곳이지. 만약 그 폐허에서 "확률적 역장의 뒤집힘과 되돌아감" 이라는 제목의 글귀를 발견하게 되면, 그것을 가지고 와주게. 그러면 관문 이동에서 자네가 생존할 확률이 급격하게 올라가게 될 것이라네.]],
	answers = {
...
newChat{ id="orb_loan",
	text = [[걱정하지 말게. 며칠 뒤에 모든 준비를 끝내고 돌려주겠네. 오, 이것을 받게. #LIGHT_GREEN#*그가 열쇠 하나를 건네줍니다.*#WHITE# 이 열쇠는 텔무르의 폐허를 여는데 쓰인다네. 오랜 세월 동안 봉인된 숄타르 인이 있는 곳이지. 만약 그 폐허에서 "확률적 역장의 뒤집힘과 되돌아감" 이라는 제목의 글귀를 발견하게 되면, 그것을 가지고 와주게. 그러면 관문 이동에서 자네가 생존할 확률이 급격하게 올라가게 될 것이라네.]],
	answers = {
		{"감사합니다. 그럼 이만.", action=function(npc, player) player:hasQuest("east-portal"):open_telmur(player) end},
	}
}
...
newChat{ id="fake_orb_end",
	text = [[나는 그런 준비 따위 하고 싶지 않네, 멍청이여. 밑을 보게.
