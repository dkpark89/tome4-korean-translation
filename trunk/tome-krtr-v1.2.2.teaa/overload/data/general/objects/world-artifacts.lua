...
클레임에는 실패했지만 언제나 살아가는 그 모습을 보고, 모든 도박사들이 그 조끼를 '무적' 이라고 부르게 되었다고 합니다.]],
	...
	special_desc = function(self) return "당신이 한번에 최대 생명력의 20%가 넘는 피해를 받으면 보호막이 펼쳐집니다. 이 보호막은 생성시 받은 피해량의 130% 만큼의 보호력을 가집니다." end,
	act = function(self)
		self:useEnergy()	
		if not self.worn_by then return end
		if game.level and not game.level:hasEntity(self.worn_by) and not self.worn_by.player then self.worn_by = nil return end
...
newEntity{ base = "BASE_LONGSWORD",
	...
	sentient=true,
	running=false,
	special_desc = function(self) return ("생명력이 20%% 아래로 떨어지면 '광란' 상태가 됩니다 (공유되는 지연시간 30 턴).") end,
	combat = {
		dam = 48,
		apr = 12,
		physcrit = 10,
...
newEntity{ base = "BASE_KNIFE", --Shibari's #1
	...
	combat = {
		...
		physcrit = 10,
		dammod = {dex=1},
		special_on_hit = {desc="뇌전이 발사되어 10칸 반경 이내의 발사체를 파괴. 파괴된 발사체 주위 5칸 반경의 적들에게 피해를 주고 기절시킴.", on_kill=1, fct=function(combat, who, target)
