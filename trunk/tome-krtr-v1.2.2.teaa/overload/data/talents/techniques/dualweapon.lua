...
newTalent{
	...
	tactical = { BUFF = 2 },
	on_pre_use = function(self, t, silent) if self:hasArcheryWeapon() or not self:hasDualWeapon() then if not silent then game.logPlayer(self, "이 기술을 사용하려면 두 개의 근접무기로 쌍수 무장을 해야 합니다.") end return false end return true end,
	getSpeed = function(self, t) return self:combatTalentScale(t, 0.11, 0.40, 0.75) end,
	activate = function(self, t)
		local weapon, offweapon = self:hasDualWeapon()
		if not weapon then
			game.logPlayer(self, "근접무기로 쌍수 무장을 하지 않으면 탄력적 공격을 사용할 수 없습니다!")
			return nil
		end
