...
newTalent{
	...
	action = function(self, t)
		...
		if target then
			if target:checkHit(self:combatSpellpower(), target:combatPhysicalResist(), 0, 95, 15) and target:canBe("instakill") and target.life > 0 and target.life < target.max_life * 0.2 then
				-- KILL IT !
				game.logSeen(target, "%s 원자 단위로 분해되었습니다!", (target.kr_name or target.name):capitalize():addJosa("가"))
				target:die(self)
			elseif target.life > 0 and target.life < target.max_life * 0.2 then
				game.logSeen(target, "%s 양자 가시를 저항했습니다!", (target.kr_name or target.name):capitalize():addJosa("가"))
			end
		end
	...
	info = function(self, t)
		local damage = t.getDamage(self, t)
		return ([[대상에게 원자 단위로 분해를 시도하여 %0.2f 시간 피해와 %0.2f 물리 피해를 줍니다. 대상이 빈사 상태가 되면 (생명력 20%% 이하) 대상은 즉사합니다.
