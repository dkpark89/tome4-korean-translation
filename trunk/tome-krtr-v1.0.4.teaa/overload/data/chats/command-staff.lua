...
local function intro(o)
	...
	if o.no_command then
		return [[아직 이 곳은 당신이 이 강력한 지팡이를 다루기 좋은 장소가 아닙니다. 만일 여기서 지팡이를 다루면 지팡이가 소멸되어 버릴 것입니다.]]
	end
...
local function how_speak(o)
	...
	local sentient_responses = {
		default = [[아, 나는 이전에 전지전능한 섬뜩한자 영매였지. 전지전능했었지만 멍청했었어, 그리고 결국 이렇게 되어버렸지. 약간의 사고로 인해 주입 기술 '쿠갈라의 영혼 역행'에 걸려버렸어. 내 상황을 요약하자면, 내 영혼은 이제 이 막대기 속에 갇혀버렸어, 그리고 내가 같이 일하던 영혼은... 뭐, 그가 어디로 가버렸는지는 자세히는 모르지만. 그러나 난 우리가 그를 만나는 일은 없었으면 하고 바라고 있어.]],
