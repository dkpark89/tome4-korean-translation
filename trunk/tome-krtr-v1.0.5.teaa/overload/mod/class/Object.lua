...
function _M:descAttribute(attr)
	...
	elseif attr == "REGEN" then
		local i = self.wielder.mana_regen or self.wielder.stamina_regen or self.wielder.life_regen or self.wielder.hate_regen or self.wielder.positive_regen_ref or self.wielder.negative_regen_ref
...
function _M:getShortName(t)
	...
	if self.quest then desc:add({"color", "VIOLET"},"[Plot Item]", {"color", "LAST"}, true) --@@ 한글화 필요
	elseif self.unique then
		if self.legendary then desc:add({"color", "FF4000"},"[Legendary]", {"color", "LAST"}, true) --@@ 한글화 필요
		elseif self.godslayer then desc:add({"color", "AAD500"},"[Godslayer]", {"color", "LAST"}, true) --@@ 한글화 필요
		else desc:add({"color", "FFD700"},"[Unique]", {"color", "LAST"}, true) --@@ 한글화 필요
		end
	end
...
function _M:getTextualDesc(compare_with)
	...
	local desc_combat = function(combat, compare_with, field, add_table)
		...
		if #dm > 0 or combat.dam then
			...
			local dtn = ( combat.damtype and (DamageType:get(combat.damtype).kr_name or DamageType:get(combat.damtype).name)) or (DamageType:get(DamageType.PHYSICAL).kr_name or DamageType:get(DamageType.PHYSICAL).name) --@@ 다음줄 사용 : 너무 길어져 변수로 뺌
	...
	if self.alchemist_bomb or self.type == "gem" and game.player:knowTalent(Talents.T_CREATE_ALCHEMIST_GEMS) then
		...
		if a then
			...
			if a.splash then desc:add(("추가적으로 %d %s 피해"):format(a.splash.dam, DamageType:get(DamageType[a.splash.type]).kr_name or DamageType:get(DamageType[a.splash.type]).name), true) end --@@ 속성이름 한글화
