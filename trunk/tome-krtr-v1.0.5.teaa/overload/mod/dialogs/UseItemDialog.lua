...
function _M:generateList()
	...
	if not self.object.__tagged and self.inven == self.actor.INVEN_INVEN then list[#list+1] = {name="버리기", action="drop"} end
	...
	if not self.object.__tagged and self.inven == self.actor.INVEN_INVEN and transmo_chest and self.actor:transmoFilter(self.object) then list[#list+1] = {name="지금 "..self.actor:transmoGetWord():capitalize(), action="transmo"} end
