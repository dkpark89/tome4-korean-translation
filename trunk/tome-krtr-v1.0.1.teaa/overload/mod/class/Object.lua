...
function _M:descAttribute(attr)
	...
	elseif attr == "CHARM" then
		return (" [세기 %d]"):format(self:getCharmPower())
...
function _M:getUseDesc()
	...
	if self.use_power then
		...
		elseif self.talent_cooldown then
			ret = tstring{{"color","YELLOW"}, ("사용처 : %s, 사용시 다른 발동형 기술들의 지연시간이 %d턴 늘어납니다."):format(util.getval((self.use_power.kr_name or self.use_power.name), self):format(self:getCharmPower()), usepower(self.use_power.power)), {"color","LAST"}}
		else
