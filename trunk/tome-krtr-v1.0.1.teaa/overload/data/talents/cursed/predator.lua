...
newTalent{
	...
	info = function(self, t)
		...
		사냥 대상으로 선택한 종류와 종족을 많이 죽일수록, 추가 효과도 더 강력해집니다. (같은 종류 살해 경험치 +0.25, 같은 종족 살해 경험치 +1) 
		살해 경험치가 %0.1f 이 되면, 추가 효과를 100%% 볼 수 있게 됩니다. 사냥 대상인 종류를 공격하면 %d%% 추가 피해를, 사냥 대상인 종족을 공격하면 %d%% 추가 피해를 주게 됩니다.%s
		기술 레벨이 상승할 때마다, 100%% 효율을 보기 위한 살해 경험치 수치가 줄어들게 됩니다.
...
newTalent{
	...
	info = function(self, t)
		...
		return ([[사냥감에 대한 학습을 통해, 사냥감의 약점을 정확히 공격합니다. 사냥감으로 지정된 종류를 공격하면 정확도가 %d 상승하며, 사냥감으로 지정된 종족을 공격하면 정확도가 %d 상승합니다. 
		또한, 근접공격 시 %0.1f%% 확률로 사냥감으로 지정된 종족을 3 턴 동안 기절시킬 수 있게 됩니다.
		기술 레벨이 상승할 때마다, 100%% 효율을 보기 위한 살해 경험치 수치가 줄어들게 됩니다.]]):format(typeAttackChange, subtypeAttackChange, subtypeStunChance)
	end,
}
...
newTalent{
	...
	info = function(self, t)
		...
		return ([[근접 공격을 할 때마다 사냥감의 허를 찔러, 물리 저항력을 %d%% 낮추고 가장 높은 능력치를 %d 감소시킵니다. 사냥감으로 지정한 종류에게는 %0.1f%% 확률, 사냥감으로 지정한 종족에게는 %0.1f%% 확률로 허를 찌릅니다. 이 효과는 %d 턴 동안 지속되며, 연속으로 효과를 발생시킬 경우 그만큼 지속시간이 연장됩니다.
...
newTalent{
	...
	info = function(self, t)
		...
		return ([[사냥감의 강점을 흉내냅니다. 사냥감으로 지정한 종족을 죽이면, 사냥감에 맞는 능력치가 상승합니다. (최대 %d 상승, 증오 수치에 따라 증감 적용) 지속시간은 무한하지만, 최근에 죽인 사냥감들에만 적용됩니다.
