...
local function forceHit(self, t, target, sourceX, sourceY, damage, knockback, knockbackDamage, power, max)
	...
	if not target.dead and knockback and knockback > 0 and target:canBe("knockback") and (target.never_move or 0) < 1 then
			...
			if not game.level.map:isBound(x, y) or is_corner_blocked or game.level.map:checkAllEntities(x, y, "block_move", target) then
				...
				if nextTarget then
					if knockbackCount > 0 then
						game.logPlayer(self, "%s %d 칸 밀려나 %s 부딪쳤습니다!", (target.kr_name or target.name):capitalize():addJosa("은"), knockbackCount, (nextTarget.kr_name or nextTarget.name):addJosa("와"))
					else
						game.logPlayer(self, "%s %s 부딪쳤습니다!", (target.kr_name or target.name):capitalize():addJosa("은"), (nextTarget.kr_name or nextTarget.name):addJosa("와"))
					end
				elseif knockbackCount > 0 then
					game.logPlayer(self, "%s %d 칸 밀려났습니다!", (target.kr_name or target.name):capitalize():addJosa("은"), knockbackCount)
				else
					game.logPlayer(self, "%s 밀려났습니다!", (target.kr_name or target.name):capitalize():addJosa("은"))
				end
		...
		if not blocked and knockbackCount > 0 then
			game.logPlayer(self, "%s %d 칸 밀려났습니다!", (target.kr_name or target.name):capitalize():addJosa("은"))
		end
