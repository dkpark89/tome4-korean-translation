...
newTalent{
	...
	--	local hit = self:attackTarget(target, nil, nil, true)
		...
		if hit then
			...
			game.logSeen(target, "%s %s 빗맞췄습니다.", (self.kr_name or self.name):capitalize():addJosa("가"), (target.kr_name or target.name):capitalize():addJosa("를"))
		end
	...
	do_throw = function(self, target, t)
		...
		if hit then
			...
			if target:isGrappled(self) and target:canBe("stun") then
				...
				game.logSeen(target, "%s 바닥에 내려꽂았습니다!", (target.kr_name or target.name):capitalize():addJosa("를"))
			-- if not grappled daze
			else
				game.logSeen(target, "%s 바닥에 넘어뜨렸습니다!", (target.kr_name or target.name):capitalize():addJosa("를"))
