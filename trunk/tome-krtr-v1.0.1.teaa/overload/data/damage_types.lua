...
setDefaultProjector(function(src, x, y, type, dam, tmp, no_martyr)
	...
	if target then
		...
		if not target.dead and affinity_heal > 0 then
			...
			local dtn = DamageType:get(type).kr_name or DamageType:get(type).name --@@ 다음줄 사용 : 너무 길어져서 변수로 뺌
			game.logSeen(target, "%s %s%s#LAST# 피해를 이용하여 생명력을 회복하였습니다!", (target.kr_name or target.name):capitalize():addJosa("가"), DamageType:get(type).text_color or "#aaaaaa#", dtn)
		end
...
newDamageType{
	...
	projector = function(src, x, y, type, dam, extra)
		...
		if realdam > 0 and target and target:attr("diseases_spread_on_blight") and (not extra or not extra.from_disease) then
			game.logSeen(src, "%s의 질병이 주위로 퍼졌습니다!", (src.kr_name or src.name):capitalize())
	...
	projector = function(src, x, y, type, dam)
		...
		if target then
			if target:checkHit((dam.power_check or src.combatSpellpower)(src), (dam.resist_check or target.combatMentalResist)(target), 0, 95, 15) then
				...
				game.logSeen(target, "%s %s의 경험치를 떨어뜨립니다!", (src.kr_name or src.name):capitalize():addJosa("가"), (target.kr_name or target.name))
			else
				game.logSeen(target, "%s 저항했습니다.", (target.kr_name or target.name):capitalize():addJosa("가"))
			end
		end
...
newDamageType{
	...
	projector = function(src, x, y, type, dam)
		...
		if target and realdam > 0 then
			...
			game.logSeen(target, "%s %s의 생명력을 빼앗아갑니다!", (src.kr_name or src.name):capitalize():addJosa("가"), (target.kr_name or target.name))
		end
		return realdam
	end,
}
...
newDamageType{
	...
	projector = function(src, x, y, type, dam)
		...
		if target and realdam > 0 then
			...
			game.logSeen(target, "%s %s의 생명력 %d 만큼을 먹어치웠습니다!", (src.kr_name or src.name):capitalize():addJosa("가"), (target.kr_name or target.name), heal) --@@ 변수 순서 조정
		end
	end,
	hideMessage=true,
}
