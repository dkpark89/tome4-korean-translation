...
newTalent{
		...
		self:project(tg, self.x, self.y, function(px, py)
			...
			if target then
				if target:canBe("fear") then
					...
					game.logSeen(target, "%s 에게 위협적인 존재감을 드러냈지만, 소용이 없었습니다!", target.name:capitalize())
				end
			end
		end)
	end,
	...
	info = function(self, t)
		...
		return ([[공격에도 꿈쩍하지 않는 당신을 보고 적들이 두려움에 빠집니다. 최대 생명력의 5%% 이상을 피해를 단번에 받으면, %d 타일 반경 이내의 적들이 두려움에 떨게 되어 물리력, 정신력, 주문력을 4 턴 동안 %d 만큼 잃게 됩니다.
	...
	do_unflinching_resolve = function(self, t)
		...
		if #effs > 0 then
			...
			if eff[1] == "effect" and rng.percent(t.getChance(self, t)) then
				...
				game.logSeen(self, "상태이상(%s) 효과에서 회복되었습니다!", self.name:capitalize())
			end
		end
	end,
	info = function(self, t)
