...
function _M:runInit(dir)
	...
	self.running = {
		...
		cnt = 1,
		dialog = Dialog:simplePopup("달리는 중...", "당신은 달려가고 있습니다. 멈추기를 원하면 Enter키를 누르세요.", function()
			self:runStop()
		end, false, true),
	}
...
function _M:runFollow(path)
	...
	if #runpath == 0 then
		game.logPlayer(self, "거기로 갈수 있는 방법을 모릅니다...")
		return
	end
	...
	self.running = {
		path = runpath,
		cnt = 1,
		dialog = Dialog:simplePopup("달리는 중...", "당신은 달려가고 있습니다. 멈추기를 원하면 아무키나 누르세요.", function()
			self:runStop()
		end, false, true),
	}
	self.running.dialog.__showup = nil
