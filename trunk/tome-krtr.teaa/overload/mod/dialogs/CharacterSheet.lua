...
function _M:init(actor)
	...
	self.actor = actor
	Dialog.init(self, "케릭터 시트: "..self.actor.name, math.max(game.w * 0.7, 950), 500)

	self.font = core.display.newFont("/data/font/soya.ttf", 15)
...
function _M:drawDialog(kind, actor_to_compare)
	...
	if player.__te4_uuid and profile.auth and profile.auth.drupid then
		...
		self:mouseLink(path, "케릭터 시트를 온라인에서도 보실 수 있습니다", s:drawColorStringBlended(self.font, LinkTxt, w, h, 255, 255, 255, true))
	end
	...
	if kind == "general" then
		...
		self:mouseTooltip(self.TOOLTIP_HEALING_MOD, s:drawColorStringBlended(self.font, ("치유 증가량 : #00ff00#%s"):format(text), w, h, 255, 255, 255, true)) h = h + self.font_h
		...
		if text then
			self:mouseTooltip(self.TOOLTIP_VISION_SEE_INVISIBLE,  s:drawColorStringBlended(self.font, ("투명화 감지  : #00ff00#%s"):format(text), w, h, 255, 255, 255, true)) h = h + self.font_h
		end
...
--			end
		...
		print_stat(self.actor.STAT_STR, "힘    ", self.TOOLTIP_STR)
		print_stat(self.actor.STAT_DEX, "민첩  ", self.TOOLTIP_DEX)
		print_stat(self.actor.STAT_MAG, "마법  ", self.TOOLTIP_MAG)
		print_stat(self.actor.STAT_WIL, "의지  ", self.TOOLTIP_WIL)
		print_stat(self.actor.STAT_CUN, "교활함", self.TOOLTIP_CUN)
		print_stat(self.actor.STAT_CON, "체격  ", self.TOOLTIP_CON)
		h = h + self.font_h
	...
	elseif kind=="attack" then
		...
		if mainhand and (#mainhand > 0) then
			...
			for i, o in ipairs(player:getInven(player.INVEN_MAINHAND)) do
				...
				if mean and mean.range then
					self:mouseTooltip(self.TOOLTIP_COMBAT_RANGE, s:drawColorStringBlended(self.font, ("원거리 (주 장비): #00ff00#%3d"):format(mean.range), w, h, 255, 255, 255, true)) h = h + self.font_h
				end
			end
		...
		if player:getInven(player.INVEN_OFFHAND) then
			for i, o in ipairs(player:getInven(player.INVEN_OFFHAND)) do
				...
				if mean and dam then
					s:drawColorStringBlended(self.font, "#LIGHT_BLUE#보조 장비:", w, h, 255, 255, 255, true) h = h + self.font_h
					text = compare_fields(player, actor_to_compare, function(actor, ...) return math.floor(actor:combatAttack(...)) end, "%3d", "%+.0f", 1, false, false, mean)
					dur_text = ("%d"):format(math.floor(player:combatAttack(o.combat)/5))
					self:mouseTooltip(self.TOOLTIP_COMBAT_ATTACK, s:drawColorStringBlended(self.font, ("정확도  : #00ff00#%s"):format(text), w, h, 255, 255, 255, true)) h = h + self.font_h
				...
				if mean and mean.range then self:mouseTooltip(self.TOOLTIP_COMBAT_RANGE, s:drawColorStringBlended(self.font, ("원거리 (보조 장비): #00ff00#%3d"):format(mean.range), w, h, 255, 255, 255, true)) h = h + self.font_h end
			end
		end
		...
		for i, ts in pairs(inc_damages) do
			if ts[1] then
				if ts[2] and ts[2] ~= ts[1] then
					self:mouseTooltip(self.TOOLTIP_INC_DAMAGE, s:drawColorStringBlended(self.font, ("%s%-20s: #00ff00#%+d%%%s(%+.0f%%)"):format((i.text_color or "#WHITE#"), i.name:capitalize().."#LAST# 피해량", ts[1] + (player.inc_damage.all or 0), ts[2] > ts[1] and "#ff0000#" or "#00ff00#", ts[1] - ts[2] ), w, h, 255, 255, 255, true)) h = h + self.font_h
				else
					self:mouseTooltip(self.TOOLTIP_INC_DAMAGE, s:drawColorStringBlended(self.font, ("%s%-20s: #00ff00#%+d%%"):format((i.text_color or "#WHITE#"), i.name:capitalize().."#LAST# 피해량", ts[1] + (player.inc_damage.all or 0)), w, h, 255, 255, 255, true)) h = h + self.font_h
				end
			else
				if ts[2] then
					self:mouseTooltip(self.TOOLTIP_INC_DAMAGE, s:drawColorStringBlended(self.font, ("%s%-20s: #00ff00#%+d%%(%+.0f%%)"):format((i.text_color or "#WHITE#"), i.name:capitalize().."#LAST# 피해량", (player.inc_damage.all or 0),-ts[2] ), w, h, 255, 255, 255, true)) h = h + self.font_h
				end
			end
		end
		...
		if player.resists_pen.all then
			...
			self:mouseTooltip(self.TOOLTIP_RESISTS_PEN_ALL, s:drawColorStringBlended(self.font, ("모든 저항 관통: #00ff00#%s"):format(text), w, h, 255, 255, 255, true)) h = h + self.font_h
		end
		...
		for i, ts in pairs(resists_pens) do
			if ts[1] then
				if ts[2] and ts[2] ~= ts[1] then
					self:mouseTooltip(self.TOOLTIP_RESISTS_PEN, s:drawColorStringBlended(self.font, ("%s%-20s: #00ff00#%+d%%%s(%+.0f%%)"):format((i.text_color or "#WHITE#"), i.name:capitalize().."#LAST# 저항 관통", ts[1] + (player.resists_pen.all or 0), ts[2] > ts[1] and "#ff0000#" or "#00ff00#", ts[1] - ts[2] ), w, h, 255, 255, 255, true)) h = h + self.font_h
				else
					self:mouseTooltip(self.TOOLTIP_RESISTS_PEN, s:drawColorStringBlended(self.font, ("%s%-20s: #00ff00#%+d%%"):format((i.text_color or "#WHITE#"), i.name:capitalize().."#LAST# 저항 관통", ts[1] + (player.resists_pen.all or 0)), w, h, 255, 255, 255, true)) h = h + self.font_h
				end
			else
				if ts[2] then
					self:mouseTooltip(self.TOOLTIP_RESISTS_PEN, s:drawColorStringBlended(self.font, ("%s%-20s: #00ff00#%+d%%(%+.0f%%)"):format((i.text_color or "#WHITE#"), i.name:capitalize().."#LAST# 저항 관통", (player.resists_pen.all or 0),-ts[2] ), w, h, 255, 255, 255, true)) h = h + self.font_h
				end
			end
		end
	...
	elseif kind=="defence" then
		...
		self:mouseTooltip(self.TOOLTIP_RDEFENSE,s:drawColorStringBlended(self.font,           ("사격 회피  : #00ff00#%s"):format(text), w, h, 255, 255, 255, true)) h = h + self.font_h
		...
		if player.resists.all then
			...
			self:mouseTooltip(self.TOOLTIP_RESIST_ALL, s:drawColorStringBlended(self.font, ("%-10s: #00ff00#%3d%% / %d%%%s"):format("모든 속성 저항", player.resists.all, player.resists_cap.all or 0, res_text), w, h, 255, 255, 255, true)) h = h + self.font_h
		end
		for i, t in ipairs(DamageType.dam_def) do
			if player.resists[DamageType[t.type]] and player.resists[DamageType[t.type]] ~= 0 then
		...
		immune_type = "disarm_immune" immune_name =    "무장해제 " if player:attr(immune_type) then text = compare_fields(player, actor_to_compare, function(actor, ...) return util.bound((actor:attr(...) or 0) * 100, 0, 100) end, "%3d%%", "%+.0f%%", 1, false, false, immune_type) self:mouseTooltip(self.TOOLTIP_SPECIFIC_IMMUNE, s:drawColorStringBlended(self.font, ("%s: #00ff00#%s"):format(immune_name, text), w, h, 255, 255, 255, true)) h = h + self.font_h end
