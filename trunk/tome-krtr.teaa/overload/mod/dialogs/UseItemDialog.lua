...
function _M:generateList()
	...
	if not self.object.__transmo then
		if self.object:canUseObject() then list[#list+1] = {name="사용", action="use"} end
		...
		if self.inven ~= self.actor.INVEN_INVEN and self.object:wornInven() then list[#list+1] = {name="착용 해제", action="takeoff"} end
	else
		list[#list+1] = {name="일반 소지품 목록으로 이동", action="toinven"}
	end
