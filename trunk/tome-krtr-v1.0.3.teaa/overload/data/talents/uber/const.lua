...
uberTalent{
	...
	trigger = function(self, t, value)
		if self.life - value < self.max_life * 0.3 and not self:isTalentCoolingDown(t) then
			...
			game.logSeen(self,"%s의 용인의 육체가 활성화되어, 생명력이 회복됩니다!",(self.kr_name or self.name) ) --I5 
		end
	end,
...
uberTalent{
	...
	info = function(self, t)
		return ([[기술이 활성화되면, 턴 소모 없이 이동할 수 있게 됩니다. 하지만 이를 위해서는 엄청난 체력이 소모되어, 1 칸 이동할 때마다 체력을 20 소모하게 됩니다.]])
		:format()
	end,
}
...
uberTalent{
	...
	info = function(self, t)
		...
		맷집과 반응속도가 크게 증가하여 최대 생명력이 150, 회피도가 %d, 모든 내성이 %d 상승합니다.
