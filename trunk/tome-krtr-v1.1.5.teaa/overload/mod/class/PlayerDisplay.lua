...
function _M:makePortrait(a, current, x, y)
	...
	self:mouseTooltip("#GOLD##{bold}#"..(a.kr_name or a.name).."\n#WHITE##{normal}#생명력 : "..math.floor(100 * a.life / a.max_life).."%\n레벨 : "..a.level.."\n"..(def.kr_title or def.title), 40, 40, x, y, function()
...
function _M:handleEffect(eff_id, e, p, ex, h)
	...
	if p.save_string and p.amount_decreased and p.maximum and p.total_dur then
		desc = ("#{bold}##GOLD#%s\n(%s: %s)#WHITE##{normal}#\n"):format(name, e.type:krEffectType(), eff_subtype)..e.long_desc(player, p).." "..("%s 이 효과의 지속시간을 %d 턴에서 %d 턴으로 %d 턴 줄였습니다."):format(p.save_string:krSaveType():addJosa("가"), p.maximum, p.total_dur, p.amount_decreased) --@ 변수 순서 조정, 임시 효과 분류 한글로 바꿈
	else
...
function _M:display()
	...
	if player.unused_stats > 0 or player.unused_talents > 0 or player.unused_generics > 0 or player.unused_talents_types > 0 then
		...
		self:makeTexture("레벨 상승!", self.w - fw, h, colors.VIOLET.r, colors.VIOLET.g, colors.VIOLET.b, fw)
	...
	if max_shield > 0 then
		self:mouseTooltip(self.TOOLTIP_DAMAGE_SHIELD, self:makeTextureBar("#WHITE#보호막 : ", nil, shield, max_shield, nil, x, h, 255, 255, 255, {r=colors.GREY.r / 3, g=colors.GREY.g / 3, b=colors.GREY.b / 3}, {r=colors.GREY.r / 6, g=colors.GREY.g / 6, b=colors.GREY.b / 6})) h = h + self.font_h
	end
	...
	if player:knowTalent(player.T_SOUL_POOL) then
		self:mouseTooltip(self.TOOLTIP_NECROTIC_AURA, self:makeTextureBar("#7fffd4#원혼 : ", "%d", player:getSoul(), player.max_soul, nil, x, h, 255, 255, 255,
	...
	if (player.unnatural_body_heal  or 0) > 0 and player:knowTalent(player.T_UNNATURAL_BODY) then
		...
		self:mouseTooltip(self.TOOLTIP_UNNATURAL_BODY, self:makeTextureBar("#c00000#잠식된 육체 : ", ("%0.1f (%0.1f/턴)"):format(player.unnatural_body_heal, math.min(regen, player.unnatural_body_heal)), regen, player.unnatural_body_heal, nil, x, h, 255, 255, 255, colors.DARK_RED, colors.VERY_DARK_RED)) h = h + self.font_h
	end
	...
	for tid, act in pairs(player.sustain_talents) do
		if act then
			...
			local displayName = t.kr_name or t.name --@ 한글 이름 추가
