...
function _M:generateList()
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=string.toTString"스크롤을 시작할 플레이어와 화면 가장자리 사이의 최소거리입니다. 이 수치가 충분히 높으면 플레이어는 항상 게임 화면의 가운데에 있게 됩니다.#WHITE#"}
	list[#list+1] = { zone=zone, name=string.toTString"#GOLD##{bold}#스크롤 거리#WHITE##{normal}#", status=function(item)
		...
		game:registerDialog(GetQuantity.new("스크롤 거리", "1 부터 30 까지", config.settings.tome.scroll_dist, 30, function(qty)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=string.toTString"백그라운드 방식으로 저장하면서, 계속 게임을 할 수 있습니다. 사용하지 않을 경우 저장하는 동안 기다려야하지만, 저장속도는 빨라집니다.#WHITE#"}
	...
	list[#list+1] = { zone=zone, name=string.toTString"#GOLD##{bold}#부드러운 이동화면#WHITE##{normal}#", status=function(item)
		...
		game:registerDialog(GetQuantity.new("이동 속도 입력(낮을수록 빠름)", "0 부터 60 까지", config.settings.tome.smooth_move, 60, function(qty)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=string.toTString"낮/밤에 따른 여러가지 조명 효과를 보여줄지 결정합니다..#WHITE#"}
	...
	list[#list+1] = { zone=zone, name=string.toTString"#GOLD##{bold}#게임화면 형식#WHITE##{normal}#", status=function(item)
		...
		Dialog:listPopup("게임화면 형식", "원하는 형식을 고르시오.", {{name="Metal", ui="metal"}, {name="Stone", ui="stone"}, {name="Simple", ui="simple"}}, 300, 200, function(sel)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=string.toTString"HUD를 어떤방식으로 보일지 결정합니다. 기본은 'Minimalist'방식입니다.\n#LIGHT_RED#이 설정의 효과는 게임을 다시 시작해야 적용됩니다."}
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=string.toTString"원래는 글꼴의 크기를 선택하는 메뉴입니다. 하지만 한글화와 한글글꼴 적용으로 현재 이 메뉴는 적용되지 않습니다. 설정을 바꿔도 게임에 영향을 주지 않습니다.\n\nSelect the fonts size.\nYou must restart the game for the change to take effect."}
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=string.toTString"지도에서 마우스를 클릭하는 곳으로 이동할것인지 결정합니다.#WHITE#"}
	...
	list[#list+1] = { zone=zone, name=string.toTString"#GOLD##{bold}#기록의 흐려짐 시간#WHITE##{normal}#", status=function(item)
		...
		game:registerDialog(GetQuantity.new("흐려짐 시간 (초)", "0 부터 20 까지", config.settings.tome.log_fade, 20, function(qty)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=string.toTString"화면에 보여줄 대화 메세지 종류를 결정합니다.#WHITE#"}
	...
	if game.uiset:checkGameOption("icons_temp_effects") then
		local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=string.toTString"상태 효과를 글자 대신 아이콘으로 표시합니다.#WHITE#"}
	...
	if game.uiset:checkGameOption("icons_hotkeys") then
		local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=string.toTString"단축 기술창에 글자 대신 아이콘으로 표시합니다.#WHITE#"}
	...
	if game.uiset:checkGameOption("hotkeys_rows") then
		local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=string.toTString"단축 기술창이 보여줄 줄 수를 결정합니다.#WHITE#"}
		list[#list+1] = { zone=zone, name=string.toTString"#GOLD##{bold}#단축 기술창 줄 수#WHITE##{normal}#", status=function(item)
			...
			game:registerDialog(GetQuantity.new("기술창 줄 수", "1 부터 4 까지", config.settings.tome.hotkey_icons_rows, 4, function(qty)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=string.toTString"단축 기술창에 보여줄 아이콘의 크기를 결정합니다.#WHITE#"}
	list[#list+1] = { zone=zone, name=string.toTString"#GOLD##{bold}#단축 기술창 아이콘 크기#WHITE##{normal}#", status=function(item)
		...
		game:registerDialog(GetQuantity.new("아이콘 크기", "32 부터 64 까지", config.settings.tome.hotkey_icons_size, 64, function(qty)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=string.toTString"새로운 게임을 시작할 때, 캐릭터의 기본 기술을 자동으로 배운상태에서 시작할지 결정합니다.#WHITE#"}
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=string.toTString"자동탐험을 하기전에 자동으로 충분한 휴식을 취할지 결정합니다.#WHITE#"}
