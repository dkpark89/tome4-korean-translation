...
function _M:drawDialog(kind, actor_to_compare)
	...
	if kind == "general" then
		...
		if player:attr("forbid_arcane") then
			s:drawColorStringBlended(self.font, "#ORCHID#지구르 추종차", w, h, 255, 255, 255, true) h = h + self.font_h
		end
		...
		if mainhand and (#mainhand > 0) then
			...
			for i, o in ipairs(player:getInven(player.INVEN_MAINHAND)) do
				...
				if mean and dam then
					...
					self:mouseTooltip(self.TOOLTIP_COMBAT_CRIT,   s:drawColorStringBlended(self.font, ("차명타율: #00ff00#%s"):format(text), w, h, 255, 255, 255, true)) h = h + self.font_h
