	...
	for i = 1, 4 do --run through list of four alchemists
		for j = 1, 3 do --run through each alchemist's list of three elixirs
			...
			elseif self:isCompleted(self.e[i][j].start) and not self:isCompleted(self.e[i][j].full) then
				desc[#desc+1] = ""..(self.e[i][j].kr_alchemist or self.e[i][j].alchemist):addJosa("는").." "..(self.e[i][j].kr_name or self.e[i][j].name):addJosa("를").." 만들기 위해 당신의 도움을 필요로 합니다. 그는 당신에게 필요한 재료들에 대한 정보를 주었습니다."
				if not self:check_i(player, self.e[i][j].ingredients[1]) then
					desc[#desc+1] = "#SLATE#  * '필요한 재료 : "..(self.e[i][j].ingredients[1].kr_name or self.e[i][j].ingredients[1].name).." 하나. "..game.party:getIngredient(self.e[i][j].ingredients[1].id).alchemy_text.."'#WHITE#"
				else
					desc[#desc+1] = "#LIGHT_GREEN#  * 당신은 "..(self.e[i][j].ingredients[1].kr_name or self.e[i][j].ingredients[1].name).." 하나를 찾았습니다.#WHITE#"
				end
				if not self:check_i(player, self.e[i][j].ingredients[2]) then
					desc[#desc+1] = "#SLATE#  * '필요한 재료 : "..(self.e[i][j].ingredients[2].kr_name or self.e[i][j].ingredients[2].name).." 하나. "..game.party:getIngredient(self.e[i][j].ingredients[2].id).alchemy_text.."'#WHITE#"
				else
					desc[#desc+1] = "#LIGHT_GREEN#  * 당신은 "..(self.e[i][j].ingredients[2].kr_name or self.e[i][j].ingredients[2].name).." 하나를 찾았습니다.#WHITE#"
				end
				if not self:check_i(player, self.e[i][j].ingredients[3]) then
					desc[#desc+1] = "#SLATE#  * 필요한 재료 : "..(self.e[i][j].ingredients[3].kr_name or self.e[i][j].ingredients[3].name).." 하나. "..game.party:getIngredient(self.e[i][j].ingredients[3].id).alchemy_text.."'#WHITE#"
				else
					desc[#desc+1] = "#LIGHT_GREEN#  * 당신은 "..(self.e[i][j].ingredients[3].kr_name or self.e[i][j].ingredients[3].name).." 하나를 찾았습니다.#WHITE#"
				end
			end
		end
	end
...
reward = function(self, player, reward)
	...
	if o then
		...
		game.logPlayer(player, "%s 받았습니다.", o:getName{do_color=true})
	end
...
competition = function(self, player, other_alchemist_nums)
	...
	self.other_alch = self.e[alch_picked][e_picked].kr_alchemist or self.e[alch_picked][e_picked].alchemist
	self.alch_picked = alch_picked
	self.other_elixir = self.e[alch_picked][e_picked].kr_name or self.e[alch_picked][e_picked].name
...
on_turnin = function(self, player, alch_picked, e_picked, player_last_elixir)
	...
	if player_last_elixir == false and self:isCompleted(self.e[alch_picked][1].full) and self:isCompleted(self.e[alch_picked][2].full) and self:isCompleted(self.e[alch_picked][3].full) then
		...
		self.winner = self.e[alch_picked][1].kr_alchemist
...
winner_is = function(self, player, alchemist_num)
	self.winner = self.e[alchemist_num][1].kr_alchemist
