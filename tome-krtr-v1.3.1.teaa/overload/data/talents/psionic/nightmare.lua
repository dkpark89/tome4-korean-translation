...
newTalent{
		...
		self:project(tg, x, y, function(tx, ty)
			...
			if target then
				if target:canBe("sleep") then
					...
					game.logSeen(self, "%s 악몽에 저항했습니다!", (target.kr_name or target.name):capitalize():addJosa("가"))
				end
			end
		end)
	...
	info = function(self, t)
		...
		return([[전방 %d 칸 반경의 적들을 %d 턴 동안 수면 상태, 그 중에서도 악몽 상태에 빠뜨립니다.
		악몽을 꾸는 동안에는 행동할 수 없게 되며, %d 피해를 받을 때마다 악몽의 지속시간이 1 턴씩 줄어들게 됩니다.
...
newTalent{
	...
	info = function(self, t)
		...
		return ([[대상에게서 내면의 악마를 끄집어냅니다.
		%d 턴 동안, 매 턴마다 %d%% 확률로 내면의 악마가 만들어집니다. 
		대상의 정신 내성이 높다면 내면의 악마가 생성되는 것에 저항할 수 있습니다.
		대상이 수면 상태라면 내면의 악마에 저항할 확률이 반이 되며, 대상의 공포 면역력을 무시할 수 있게 됩니다.
		만약 내면의 악마 생성이 실패한다면 주문이 일찍 끝나버립니다.
...
newTalent{
	...
	action = function(self, t)
		...
		if target:canBe("fear") or target:attr("sleep") then
			...
			game.logSeen(target, "%s 악몽에 저항했습니다!", (target.kr_name or target.name):capitalize():addJosa("가"))
		end
	...
	info = function(self, t)
		...
		대상이 수면 상태라면 상태 이상효과에 저항할 확률이 반이 되며, 대상의 공포 면역력을 무시할 수 있게 됩니다.
