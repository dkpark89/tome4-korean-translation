...
newTalent{
	...
	info = function(self, t)
		...
		return ([[%d%% 의 무기 피해를 주는 화살을 발사한 후에, 만약 충분한 공간이 있다면, 감시자들을 2명까지 불러냅니다. 감시자들은 그들의 시간축으로 돌아가기 전까지 하나의 화살을 각각 발사 할 수 있습니다.
		감시자들은 현실에서 벗어나 있기 때문에 %d%% 만큼 낮은 피해를 입히지만 모든 공격이 아군을 지나가게 됩니다. 이제 모든 당신의 화살들은, 사격 혹은 다른 기술으로 부터 발사 된 것을 포함해, 위상을 변화 시켜 아군들을 피해 입히지 않고 뚫고 지나게 됩니다.  
