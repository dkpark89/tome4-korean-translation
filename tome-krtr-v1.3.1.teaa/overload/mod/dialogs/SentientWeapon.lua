...
--	print("Incoming factory setting maxes:", self.o.factory_settings.maxes.dam)-- self.o.factory_settings.maxes.critical_power, self.o.factory_settings.maxes.max_acc)
	...
	Dialog.init(self, (self.o.kr_name or self.o.name), 300, 200)
	...
	for tid, act in pairs(self.actor.sustain_talents) do
		if act then
			...
			if t.no_sustain_autoreset then
				game.logPlayer(self.actor, "#LIGHT_BLUE#주의: 당신의 능력치가 향상되었습니다. 바뀐 능력치의 영향을 받는 기술 %s 현재 유지중입니다. 변경된 효과를 적용하기 위해서는 기술을 다시 사용할 필요가 있습니다.", (t.kr_name or t.name):addJosa("는"))
			else
				reset[#reset+1] = tid
			end
		end
...
function _M:incStat(v, id)
	...
	if v == 1 then
		...
		if self.o.wielder[id] >= self.o.factory_settings.maxes[id] then
			self:simplePopup("레벨 한계치", "레벨이 더 올라야 이 능력치를 올릴 수 있습니다!")
			return
		end
	else
