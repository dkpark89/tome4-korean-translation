...
newEntity{ base = "BASE_STAFF",
	...
	use_power = { name = "cure diseases", kr_name = "질병 치료", power = 10,
		use = function(self, who)
			...
			game.logSeen(who, "%s의 질병이 치료되었습니다!", (who.kr_name or who.name):capitalize())
...
newEntity{ base = "BASE_GAUNTLETS",
	...
	kr_name = "폭풍을 부르는 자의 전투장갑", kr_unided_name = "잘 맞물린 전투장갑",
...
newEntity{ base = "BASE_MINDSTAR",
	...
	kr_name = "진흙 덩어리 심장", kr_unided_name = "찐득한 마석",
	level_range = {27, 34},
...
newEntity{ base = "BASE_WHIP",
	...
	use_power = { name = "strike an enemy in range 3, releasing a burst of lightning", kr_name = "주변 3 칸 반경의 적 하나를 공격하여, 전기 폭발 발생", power = 10,
		use = function(self, who)
			...
			game.logSeen(who, "%s %s 공격해, 전류를 흘려보냅니다!", (who.kr_name or who.name):capitalize():addJosa("가"), (target.kr_name or target.name):addJosa("를"))
			return {id=true, used=true}
		end
	},
}
	...
	summon=function(o, who)
		...
		local x, y = util.findFreeGrid(who.x, who.y, 5, true, {[engine.Map.ACTOR]=true})
			...
			local m = NPC.new{
				...
				desc = "코르파투스가 폭발하면서 나온 부패한 슬라임 덩어리로, 당신을 죽이려고 하는 것 같습니다.",
...
newEntity{ base = "BASE_LONGSWORD",
	...
	kr_name = "적의없음", kr_unided_name = "뒤틀린 칼날",
	...
	act = function(self)
		...
		local who = self.worn_by
			...
			who:project(blast, who.x, who.y, function(px, py)
				...
				game.logSeen(who, "칼날의 눈이 %s의 정신을 꿰뚫어봅니다!", (target.kr_name or target.name):capitalize())
			end)
	end,
...
newEntity{ base = "BASE_WHIP", define_as = "HYDRA_BITE",
	...
	combat = {
		...
		special_on_hit = {desc="인접한 두 적을 동시에 공격",on_kill=1, fct=function(combat, who, target)
				...
				if twohits then
					game.logSeen(who, "%s의 머리 셋 달린 도리깨가 %s %s 후려칩니다!", (who.kr_name or who.name):capitalize(), (target1.kr_name or target1.name):capitalize():addJosa("와"),(target2.kr_name or target2.name):capitalize():addJosa("를"))
				else
					game.logSeen(who, "%s의 머리 셋 달린 도리깨가 %s 후려칩니다!", (who.kr_name or who.name):capitalize(), (target1.kr_name or target1.name):capitalize():addJosa("를"))
				end
...
newEntity{ base = "BASE_GAUNTLETS",
	...
	power_up= function(self, who, level)
		...
		self.use_power.kr_name = "5칸 이내 원뿔영역의 마법 파괴"
...
newEntity{ base = "BASE_SHIELD", --Thanks SageAcrin!
	...
	on_block = function(self, who, target, type, dam, eff)
		if rng.percent(30) then
			...
			game.logSeen(who, "%s의 방패에서 차가운 물줄기가 분출되어, %s에게 발사됩니다!", (who.kr_name or who.name):capitalize(), (target.kr_name or target.name):capitalize())
		end
	end,
}
...
newEntity{ base = "BASE_GREATMAUL",
	...
	combat = {
		...
		special_on_hit = {desc="25% 확률로 마법 보호막 분쇄", fct=function(combat, who, target)
			if not rng.percent(25) then return end
			if not target then return end
