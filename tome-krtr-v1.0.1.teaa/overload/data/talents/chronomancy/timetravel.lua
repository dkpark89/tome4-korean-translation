...
newTalent{
	...
	action = function(self, t)
		...
		if target:attr("timetravel_immune") then
			game.logSeen(target, "%s의 시간은 지울 수 없습니다!", (target.kr_name or target.name):capitalize())
			return
		end
		...
		game.logSeen(target, "%s 시간의 흐름에서 벗어났습니다!", (target.kr_name or target.name):capitalize():addJosa("가"))
