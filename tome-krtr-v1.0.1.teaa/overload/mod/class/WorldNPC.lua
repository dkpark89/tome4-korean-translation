...
function _M:takePowerHit(val, src)
	...
	if self.unit_power <= 0 then
		game.logSeen(self, "%s %s 죽였습니다.", (src.kr_name or src.name):capitalize():addJosa("가"), (self.kr_name or self.name):addJosa("를"))
		self:die(src)
	end
end
...
function _M:encounterAttack(target, x, y)
	...
	if self.unit_power <= 0 then
		game.logSeen(self, "%s %s 죽였습니다.", (target.kr_name or target.name):capitalize():addJosa("가"), (self.kr_name or self.name):addJosa("를"))
		self:die(target)
	end
	if target.unit_power <= 0 then
		game.logSeen(target, "%s %s 죽였습니다.", (self.kr_name or self.name):capitalize():addJosa("가"), (target.kr_name or target.name):addJosa("를"))
		target:die(src)
	end
end
