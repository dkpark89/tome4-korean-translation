...
newTalent{
	...
	info = function(self, t)
		...
		return ([[영혼에 대한 시전자의 갈망이 더 강해집니다. 적을 살해할 때마다 적의 원한을 끄집어내, %d%% 확률로 하나의 원혼을 더 얻을 수 있게 됩니다.
...
newTalent{
	...
	action = function(self, t)
		...
		self:project(tg, x, y, function(px, py)
			...
			if 100 * m.life / m.max_life <= t.getMaxLife(self, t) and self:checkHit(self:combatSpellpower(), m:combatSpellResist()) and m:canBe("instakill") and m.rank <= 3.2 and not m:attr("undead") and not m.summoner and not m.summon_time then
				...
				self:logCombat(m, "#GREY##Source# #target#의 영혼을 제거하여, 언데드 하수인으로 만들었습니다.")
			end
		end)
	...
	info = function(self, t)
		...
		성공할 경우 원혼을 2 얻게 되며, 대상은 시전자의 (사령술의 기운에 영향을 받지 않는) 영구적인 하수인이 됩니다.
		대상의 육신은 생전의 능력을 그대로 사용할 수 있으며, 어둠의 공감 기술 역시 적용됩니다.
		대상의 육신을 하수인으로 만들면 대상의 생명력이 완전히 회복되지만, 이후 어떤 방법으로도 대상의 생명력은 회복되지 않습니다.
		한번에 하나의 육신만을 조종할 수 있습니다. 만약 이 주문을 다시 사용하면 (심지어 새로운 하수인이 만들어지지 않더라도) 기존의 하수인은 사라지게 됩니다.
...
newTalent{
	...
	info = function(self, t)
		...
		return ([[하수인이 자폭합니다. 주변 %d 칸 반경으로 그림자 폭발이 일어나, 범위 내에 있는 모든 적들에게 %0.2f 어둠 피해를 줍니다.
		하수인의 주인이 죽었을 경우에만 이 기술을 사용할 수 있습니다.]]):format(rad, damDesc(self, DamageType.DARKNESS, 50 + 10 * self.level))
	end,
}
