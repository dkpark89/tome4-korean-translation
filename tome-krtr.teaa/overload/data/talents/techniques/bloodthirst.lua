...
newTalent{
	...
	info = function(self, t)
		...
		return ([[적들의 피를 흩뿌리며 환희합니다. 적에게 치명타를 가하면, 최대 생명력이 %d%% , 턴 당 생명력 재생이 %0.2f , 체력 재생이 %0.2f 증가됩니다.
		생명력과 체력 재생 증가는 5 회까지 중첩되며, 최대치는 %0.2f , %0.2f 입니다.]]):
...
newTalent{
	...
	do_terror = function(self, t, target, dam)
		...
		if target:canBe("stun") then
			...
			game.logSeen(target, "%s 에게 죽음의 공포를 안겨줬지만, 저항했습니다!", target.name:capitalize())
		end
	end,
	...
	info = function(self, t)
		return ([[강렬한 타격으로 적을 극심한 공포에 떨게 만듭니다. 적의 최대 생명력을 %d%% 이상 감소시키는 타격을 가하면, 대상은 극심한 공포에 빠져 5 턴 동안 혼절합니다.
...
newTalent{
	...
	info = function(self, t)
		return ([[적의 눈에 흙이 들어갈 때마다 당신에게 강력한 힘이 솟구쳐, 최대 %d 까지 힘이 2씩 증가합니다.]]):
...
newTalent{
	...
	info = function(self, t)
		return ([[%d 턴 동안 전투 광란 상태가 됩니다. 효과가 지속되는 동안에는 물품을 사용할 수도 없고 치유를 받을 수도 없으며, 생명력이 1 밑으로 떨어지지 않습니다.
