...
newTalent{
	...
	info = function(self, t)
		return ([[한바퀴 회전하면서, 근접한 주변의 적들에게 %d%% 의 무기 피해를 입힙니다.]]):format(100 * self:combatTalentWeaponDamage(t, 1.4, 2.1))
	end,
}
...
newTalent{
	...
	points = 5,
	random_ego = "attack",
	message = function(self) if self.subtype == "rodent" then return "대상이(@Source@) 전투의 찍찍거림을 사용합니다." else return "대상(@Source@)이 전투의 외침을 사용합니다." end end ,
	stamina = 30,
	cooldown = 18,
	...
	info = function(self, t)
		return ([[전방 %d 타일 반경에 전투의 외침을 내지릅니다. 범위 내의 대상들은 %d 턴 동안 혼란 상태에 빠집니다.]]):
	...
	action = function(self, t)
		...
		if hit then
			...
			elseif target.life > 0 and target.life < target.max_life * 0.2 then
				game.logSeen(target, "%s 에게 죽음의 일격을 내리쳤지만, 저항했습니다!", target.name:capitalize())
			end
		end
		return true
	end,
	...
	action = function(self, t)
		...
		if hit then
			if target:canBe("stun") then
				...
				game.logSeen(target, "%s 에게 기절시키기를 사용했지만, 저항했습니다!", target.name:capitalize())
			end
		end
...
newTalent{
	...
	action = function(self, t)
		local weapon = self:hasTwoHandedWeapon()
		if not weapon then
			game.logPlayer(self, "양손 무기 없이는 팔 부수기를 쓸 수 없습니다!")
			return nil
		end
	...
	info = function(self, t)
		return ([[대상의 팔을 무기로 내리쳐서 %d%% 의 무기 피해를 주고, 공격에 성공하면 대상의 정확도가 %d 감소하는 효과를 %d 턴 동안 지속되게 합니다.
