...
newTalent{
	...
	cooldown = 10,
	stamina = 12,
	message = "대상(@Source@)이 마무리 올려치기를 날렸습니다.",
	...
	action = function(self, t)
		...
		if hit then
			if target:canBe("stun") then
				...
				game.logSeen(target, "%s 에게 마무리 올려치기를 날렸지만, 기절 효과에 저항했습니다!", target.name:capitalize())
			end
		end
...
newTalent{
	...
	cooldown = 10,
	stamina = 10,
	message = "대상(@Source@)이 충격타를 날렸습니다.",
	...
	info = function(self, t)
		...
		return ([[강력한 충격이 실린 펀치로 대상을 가격하여 %d%% 의 피해를 주고, 공격이 성공하면 연계 점수에 따라 %d 타일 반경 내의 모든 대상에게 %0.2f - %0.2f 의 물리 피해를 줍니다.
...
newTalent{
	...
	cooldown = 10,
	stamina = 10,
	message = "대상(@Source@)이 몸통 치기를 날렸습니다.",
	tactical = { ATTACK = { weapon = 2 }, DISABLE = { stun = 2 } },
	requires_target = true,
	--on_pre_use = function(self, t, silent) if not self:hasEffect(self.EFF_COMBO) then if not silent then game.logPlayer(self, "You must have a combo going to use this ability.") end return false end return true end,
	getDamage = function(self, t) return self:combatTalentWeaponDamage(t, 1.1, 1.8) + getStrikingStyle(self, dam) end,
	...
	action = function(self, t)
		...
		if hit then
			...
			if target:canBe("stun") then
				...
				game.logSeen(target, "%s 에게 몸통 치기를 사용했지만, 저항했습니다!", target.name:capitalize())
			end
...
newTalent{
	...
	cooldown = 16,
	stamina = 12,
	message = "대상(@Source@)이 죽음의 강타를 날립니다!",
	tactical = { ATTACK = { weapon = 2 } },
	requires_target = true,
	--on_pre_use = function(self, t, silent) if not self:hasEffect(self.EFF_COMBO) then if not silent then game.logPlayer(self, "You must have a combo going to use this ability.") end return false end return true end,
	...
	action = function(self, t)
		...
		if hit then
			...
			elseif target.life > 0 and target.life < target.max_life * 0.2 then
				game.logSeen(target, "%s 에게 죽음의 강타를 후려쳤지만, 저항했습니다!", target.name:capitalize())
			end
		end
