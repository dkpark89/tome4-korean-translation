...
newTalent{
	...
	info = function(self, t)
		...
		return ([[당신의 신체 크기보다 한 단계 큰 대상까지, %d 턴 동안 붙잡아 안습니다. 붙잡힌 대상은 이동할 수 없게 되고 정확도와 회피도가 %d 감소됩니다. 붙잡은 상태에서 이동하게 되면 기술이 풀립니다. 붙잡기를 유지하는 동안에는 매 턴마다 체력이 %d 씩 소모됩니다.
			...
			if hit then
				if grappled then
					...
					if target:canBe("stun") then
						...
						game.logSeen(target, "%s 에게 넘어뜨리기를 사용했지만, 기절 효과에 저항했습니다!", target.name:capitalize())
					end
				else
					...
					if target:canBe("stun") then
						...
						game.logSeen(target, "%s 에게 넘어뜨리기를 사용했지만, 혼절 효과에 저항했습니다!", target.name:capitalize())
					end
				end
			end
		end
