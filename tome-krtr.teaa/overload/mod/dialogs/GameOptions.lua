...
function _M:generateList()
	...
	list[#list+1] = { zone=zone, name=string.toTString"#GOLD##{bold}#스크롤 거리#WHITE##{normal}#", status=function(item)
		...
		game:registerDialog(GetQuantity.new("스크롤 거리", "1 부터 20 까지", config.settings.tome.scroll_dist, 20, function(qty)
	...
	list[#list+1] = { zone=zone, name=string.toTString"#GOLD##{bold}#HUD 형식#WHITE##{normal}#", status=function(item)
		...
		Dialog:listPopup("HUD 형식", "원하는 형식을 고르시오.", {{name="최소", ui="Minimalist"}, {name="고전", ui="Classic"}}, 300, 200, function(sel)
			if not sel or not sel.ui then return end
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=string.toTString"원래는 글꼴을 선택하는 메뉴입니다. 하지만 한글화와 한글폰트 적용으로 현재 이 메뉴는 적용되지 않습니다. 설정을 바꿔도 게임에 영향을 주지 않습니다.\n\nSelect the fonts look. Fantasy is the default one. Basic is simplified and smaller.\nYou must restart the game for the change to take effect."}
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=string.toTString"원래는 글꼴의 크기를 선택하는 메뉴입니다. 하지만 한글화와 한글폰트 적용으로 현재 이 메뉴는 적용되지 않습니다. 설정을 바꿔도 게임에 영향을 주지 않습니다.\n\nSelect the fonts size.\nYou must restart the game for the change to take effect."}
