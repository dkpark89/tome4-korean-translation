...
function _M:handleEffect(eff_id, e, p, ex, h)
	...
	if p.save_string and p.amount_decreased and p.maximum and p.total_dur then
		desc = ("#{bold}##GOLD#%s\n(%s: %s)#WHITE##{normal}#\n"):format(name, e.type, eff_subtype)..e.long_desc(player, p).." "..("%s 이 효과의 지속시간을 %d에서 %d로 %d턴 만큼 줄였습니다..":format((p.save_string):addJosa("가"), p.maximum, p.total_dur, p.amount_decreased) --"%s reduced the duration of this effect by %d turns, from %d to %d."
	else
...
function _M:display()
	...
	self:mouseTooltip(self.TOOLTIP_GOLD, self:makeTexture(("소지금: #00ff00#%0.2f"):format(player.money or 0), x, h, 255, 255, 255)) h = h + self.font_h

	--Display attack, defense, spellpower, mindpower, and saves.
	local attack_stats = {{"combatAttack", "TOOLTIP_COMBAT_ATTACK", "정확도  :"}, {"combatPhysicalpower", "TOOLTIP_COMBAT_PHYSICAL_POWER", "물리력  :"}, {"combatSpellpower", "TOOLTIP_SPELL_POWER", "주문력  :"}, {"combatMindpower", "TOOLTIP_MINDPOWER", "정신력  :"}, {"combatDefense", "TOOLTIP_DEFENSE", "회피도  :"}, {"combatPhysicalResist", "TOOLTIP_PHYS_SAVE", "물리내성:"}, {"combatSpellResist", "TOOLTIP_SPELL_SAVE", "주문내성:"}, {"combatMentalResist", "TOOLTIP_MENTAL_SAVE", "정신내성:"}}
