...
function _M:restCheck()
	...
	if #spotted > 0 then
		...
		return false, ("%s의 적대적 존재 발견 (%s%s)"):format(dir or "???", (spotted[1].actor.kr_name or spotted[1].actor.name), game.level.map:isOnScreen(spotted[1].x, spotted[1].y) and "" or " - 화면바깥")
	end
...
function _M:doDrop(inven, item, on_done, nb)
	...
	if o and o.__tagged then
		game.logPlayer(self, "당신은 %s 버릴 수 없습니다 (tagged).", o:getName{do_colour=true}:addJosa("를")) --@@ 한글화 필요
		return
	end
...
function _M:useOrbPortal(portal)
	...
	if #spotted > 0 then
		local dir = game.level.map:compassDirection(spotted[1].x - self.x, spotted[1].y - self.y)
		self:logCombat(spotted[1].actor, "시야 내에 적이 있을 경우, 오브를 사용할 수 없습니다 (#Target# : %s%s).",dir, game.level.map:isOnScreen(spotted[1].x, spotted[1].y) and "" or " - 화면바깥")
		return
	end
