...
newInscription{
	...
	info = function(self, t)
		...
		하지만 생명력이 0 이하로 떨어지면 남은 생명력을 알 수 없게 되며, 영웅 효과가 끝나기 전에 반드시 회복을 해야 합니다.]]):format(data.dur, data.power + data.inc_stat, data.die_at + data.inc_stat * 30) --@@ 변수 순서 조정
	end,
	short_info = function(self, t)
		local data = self:getInscriptionData(t.short_name)
...
newInscription{
	...
	short_info = function(self, t)
		local data = self:getInscriptionData(t.short_name)
		return ([[%d 피해 반사, %d 턴 유지]]):format(100+3*self:getMag(), 5)
	end,
}
