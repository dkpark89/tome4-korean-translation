...
--		level_range = {1, 50}, exp_worth=0,
		...
		canWearObjectCustom = function(self, o)
			...
			if not self.summoner:knowTalent(self.summoner.T_GEM_GOLEM) then return "룬 골렘 기술이 필요함" end
			if not o.material_level then return "이 보석은 사용할 수 없음" end
			if o.material_level > self.summoner:getTalentLevelRaw(self.summoner.T_GEM_GOLEM) then return "이 보석을 사용하기에는 룬 골렘 기술 레벨이 부족" end
		end,
