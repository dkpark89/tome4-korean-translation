	...
	action = function(self, t)
		...
		self:project(tg, x, y, function(px, py)
			...
			if self:reactionToward(target) < 0 then
				...
				self:logCombat(target, "#Source2# #Target3# 도발하여, 자신만을 공격하도록 만들었습니다.")
			end
		end)
		return true
	end,
	info = function(self, t)
		return ([[골렘이 주변 %d 칸 반경의 적들을 도발하여, 골렘을 공격하도록 만듭니다.]]):format(self:getTalentRadius(t)) 
	end,
}
...
newTalent{
	...
	action = function(self, t)
		...
		for i, target in ipairs(tgts) do
			...
			self:logCombat(target.actor, "#Target1# #Source#에 의해 끌려옵니다!")
