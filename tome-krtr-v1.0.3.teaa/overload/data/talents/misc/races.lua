...
newTalent{
	...
	info = function(self, t)
		...
		또한 마법을 사용하여 피해를 줄 때마다, 해당 속성의 피해량이 5 턴 동안 15%% 증가하게 됩니다. (이 효과에는 재사용 대기시간이 있습니다)]]): --@@ 한글화 필요
...
newTalent{
	...
	info = function(self, t)
		...
		속도 증가량은 민첩 능력치와 마법 능력치 중 높은 쪽의 영향을 받아 증가합니다.]]):format((0.1 + math.max(self:getDex(), self:getMag()) / 210) * 100)
	end,
}
...
newTalent{
	...
	info = function(self, t)
		return ([[샬로레 종족의 마법적 특성으로 인해, 현실이 약간 왜곡되어 모든 치명타율이 %d%% / 치명타 배수가 %d%% 상승합니다.]]):format(self:getTalentLevelRaw(t) * 2, self:getTalentLevelRaw(t) * 5)
	end,
}
...
newTalent{
	...
	info = function(self, t)
		...
		나쁜 상태효과의 지속시간은 %d 턴 줄어들고, 기술의 지연시간이 %d 턴 짧아지며, 좋은 상태효과의 지속시간은 %d 턴 늘어납니다.]]):
