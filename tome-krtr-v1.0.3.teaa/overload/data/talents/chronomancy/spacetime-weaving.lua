	...
	action = function(self, t)
		...
		self:project(tg, self.x, self.y, function(px, py)
			...
			if self:checkHit(power, target:combatSpellResist() + (target:attr("continuum_destabilization") or 0)) and target:canBe("teleport") then
				...
				game.logSeen(target, "%s 추방을 저항했습니다!", (target.kr_name or target.name):capitalize():addJosa("가"))
			end
		end)
...
newTalent{
	...
	action = function(self, t)
				...
				if who == self.summoned_by or who:checkHit(self.check_hit, who:combatSpellResist(), 0, 95, 15) and who:canBe("teleport") then
					...
					if tx and ty then
						if not who:teleportRandom(tx, ty, 0) then
							game.logSeen(who, "%s 웜홀로 들어가려 했으나, 난폭한 힘이 밖으로 밀어냈습니다.", (who.kr_name or who.name):capitalize():addJosa("가"))
						elseif who ~= self.summoned_by then
							who:setEffect(who.EFF_CONTINUUM_DESTABILIZATION, 100, {power=self.destabilization_power})
						end
					end
				else
					game.logSeen(who, "%s 웜홀을 무시했습니다.", (who.kr_name or who.name):capitalize():addJosa("가"))
				end
				return true
			end,
			act = function(self)
		...
		game.logSeen(self, "%s 두 지점 사이의 공간을 접습니다.", (self.kr_name or self.name):capitalize():addJosa("가"))
		return true
	end,
	info = function(self, t)
...
newTalent{
	...
	info = function(self, t)
		...
		그리고 시공 계열 마법으로 적을 공격할 때 추방, 시간 지우기 등 '연속체 불안정화' 효과가 일어날 확률을 높이고, 자신의 연속체 불안정화 면역 확률을 %d%% 증가시킵니다.]]):
