...
newTalent{
	...
	action = function(self, t)
		...
		if self:checkHit(self:combatSpellpower(), target:combatPhysicalResist()) then
			...
			game.logSeen(target, "%s 돌풍에 밀려났습니다!", (target.kr_name or target.name):capitalize():addJosa("가"))
			target:crossTierEffect(target.EFF_OFFBALANCE, self:combatSpellpower())
		else
			game.logSeen(target, "%s 돌풍을 정면으로 맞고도, 꿈쩍도 하지 않았습니다!", (target.kr_name or target.name):capitalize():addJosa("가"))
		end
		return true
	end,
	info = function(self, t)
...
newTalent{
	...
	action = function(self, t)
		...
		if self:checkHit(self:combatMindpower(), target:combatPhysicalResist()) then
			target:knockback(self.x, self.y, 1)
			game.logSeen(target, "%s 주먹에 얻어맞아 밀려났습니다!", (target.kr_name or target.name):capitalize():addJosa("가"))
			target:crossTierEffect(target.EFF_OFFBALANCE, self:combatMindpower())
		else
			game.logSeen(target, "%s 주먹의 영향을 받지 않았습니다!", (target.kr_name or target.name):capitalize():addJosa("가"))
		end
		return true
	end,
	info = function(self, t)
...
newTalent{
	...
	action = function(self, t)
		...
		if self:checkHit(self:combatSpellpower(), target:combatSpellResist()) then
			target:knockback(self.x, self.y, 1)
			game.logSeen(target, "%s 순간이동 되었습니다!", (target.kr_name or target.name):capitalize():addJosa("가"))
			target:crossTierEffect(target.EFF_SPELLSHOCKED, self:combatSpellpower())
		else
			game.logSeen(target, "%s 순간이동을 저항했습니다!", (target.kr_name or target.name):capitalize():addJosa("가"))
		end
		return true
	end,
	info = function(self, t)
...
newTalent{
	...
	action = function(self, t)
		...
		if self:checkHit(self:combatMindpower(), target:combatMentalResist()) then
			target:knockback(self.x, self.y, 1)
			game.logSeen(target, "%s 공포에 질려 도망갔습니다!", (target.kr_name or target.name):capitalize():addJosa("가"))
			target:crossTierEffect(target.EFF_BRAINLOCKED, self:combatMindpower())
		else
			game.logSeen(target, "%s 공포감이 들어 살짝 몸을 떨었습니다.", (target.kr_name or target.name):capitalize():addJosa("가"))
		end
		return true
	end,
	info = function(self, t)
