...
newChat{ id="more_ex",
	text = [[#LIGHT_GREEN#*당신은 남자가 후드 안에서 웃고 있다는 것을 느꼈습니다.*#WHITE#
투기장은 용감한 자들이 다른 모든 것들과 싸우는 장소다. 
우리는 계속 그 규모를 확장시키고 있고, 그래서 언제나 도전자가 부족하지... 
간단하게, 도박이라고 보면 편할거다. 돈 대신 네 전투력을 사용할 뿐이지. 이해되나?
우리는 좋은 구경거리를 만들어주는 자가 필요하다는 얘기지. 
물론 그런 자에게는 보답으로... 그 자식 세대까지 전해질 부와 명예를 얻을 수 있는거고.
내가 주는 간단한 시험을 통과한다면... 너를 #LIGHT_RED#투기장에 들어갈 수 있게 해주지#WHITE#. 
물론 지금 당장 투기장에 들어갈 필요는 없네. 네 모험을 끝내고, 그때 와도 상관 없다네. 
거기에 내 제안을 받아들인다면, 네가 필요할 #LIGHT_RED#전투 경험#WHITE#까지 전수해주도록 하지...
그래서, 네 결정은? 이제 할 마음이 생겼나?
]],
	answers = {
		{"싸울 준비는 끝났다. 시작하지!", jump="accept", action = function (self, player) self.talked_to = 2 end },
...
그것 참 불행한 일이로군. 너같은 사람이 필요했는데 말이야. 
너는 정말 관중들이 좋아하는 타입이야. 너라면 충분히 투기장의 지배자가 될 수 있을텐데.
아아, 네가 그렇게 결정한다면, 우리는 다시 만날 수 없을거야. 
하지만, 만약 마음이 바뀐다면... #YELLOW#데르스에 조금 더 오래 머물러 있도록 하지.#WHITE#
...
좋아! 진정한 싸움꾼은 언제나 싸울 준비가 되어 있는 법이지. 
우리를 만난 것을 절대 후회하지 않게 될거야...
그래, 싸울 준비는 됐지?
]],
	answers = {
		{"좋아, 준비 됐어!", jump="go"},
...
내 이름은 '레즈' 라고 하네. 나는 투기장에서 좋은 볼거리를 만들어줄 뛰어난 싸움꾼들을 고용하고 있지... 
두 방 맞고 죽어버리지 않을 그런 녀석들 말이야. 그리고 너 역시 이제 그들 중 하나가 된거고!
나는 네 모험을 방해할 생각은 없어. 나 역시 모험가였거든, 오래 전에는 말이지. 
하지만 우리는 너를 진정한 챔피언으로 만들어줄 수 있어. 많은 자들의 사랑을 받고 다이아몬드로 목욕할 수 있는 사람으로 말이지.
...
#WHITE#좋아, @playername@. 이제 나는 가봐야 할 시간이야. 그대의 모험에 행운이 있기를. 그리고 모험이 끝나면 언제든지 우리 투기장에 들러주게!
	...
	answers = {
		{ "그러죠. 그동안 잘 지내시길.", action = function (self, player) game:onLevelLoad("arena-unlock-1", function()
