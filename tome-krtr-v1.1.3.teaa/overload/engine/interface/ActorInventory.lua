...
function _M:dropFloor(inven, item, vocal, all)
	...
	if vocal then game.logSeen(self, "%s %s 바닥에 버립니다.", (self.kr_name or self.name):capitalize():addJosa("이"), o:getName{do_color=true}:addJosa("를")) end
...
function _M:findInInventory(inven, name, getname)
	...
	for item, o in ipairs(inven) do
		if (o:getOriName(getname) == name or o:getName(getname) == name or o.name == name) then return o, item end --@ 원래이름도 비교되도록 코드 수정, 마지막은 예비용
	end
end
