...
newEntity{ base = "BASE_NPC_HORROR", define_as = "BASE_NPC_ELDRICTH_EYE",
	...
	on_die = function(self, src)
		...
		self:logCombat(self.summoner, "#AQUAMARINE# #Source1# 쓰러지자 #Target#도 약해진 것 같습니다!")
...
newEntity{ base="BASE_NPC_HORROR", define_as = "GRGGLCK_TENTACLE",
	...
	on_act = function(self)
		if self.summoner.dead then
			self:die()
			self:logSeen(self, "#AQUAMARINE##Source1# 쓰러지자, 당신은 #Target1# 고통에 몸서리 치는 것을 발견합니다!")
		end
	end,

	on_die = function(self, who)
		if self.summoner and not self.summoner.dead and who then
			game.logSeen(self, "#AQUAMARINE#%s 쓰러지자, %s 고통에 몸부림을 칩니다!", (self.kr_name or self.name):addJosa("가"), (self.summoner.kr_name or self.summoner.name):addJosa("가"))
