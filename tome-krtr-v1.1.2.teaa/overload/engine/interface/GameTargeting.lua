...
function _M:targetMode(v, msg, co, typ)
	...
	if not v then
		...
		if msg then self.log(type(msg) == "string" and msg or "전략적 화면이 비활성화됩니다. 활성화하려면 'Shift + T' 를 누르세요.") end
		...
		if msg then self.log(type(msg) == "string" and msg or "전략적 화면이 활성화됩니다. 비활성화하려면 'Shift + T' 를 누르세요.") end
		self.level.map.changed = true
