...
newTalent{
	...
	do_instakill = function(self, t)
		...
		if tg then
			game:onTickEnd(function()
				...
				game.logSeen(tg, "#LIGHT_BLUE#%s 따위는 원래부터 없었던 존재였습니다!", (tg.kr_name or tg.name):capitalize())
				tg:die(self)
			end)
		end
	end,
	action = function(self, t)
		...
		if target == self then
			game.logSeen(self, "#LIGHT_STEEL_BLUE#%s 자기 자신의 존재를 지우려고 합니다!", (self.kr_name or self.name):addJosa("가"))
			self:incParadox(400)
	...
	info = function(self, t)
		...
		return ([[%d 턴 동안, 대상을 시공간에서 없애버리려고 시도합니다. 지속시간 동안 대상의 전체 저항력이 %d%% 감소합니다. 
...
newTalent{
	...
	info = function(self, t)
		...
		return ([[10 턴 동안 시공간에서 자신의 일부분을 없앱니다.
		그 결과 전체 저항력이 %d%% 상승하고 모든 나쁜 상태이상 효과의 지속시간이 %d%% 줄어들지만, 적에게 가하는 모든 공격의 피해량이 20%% 감소합니다.
