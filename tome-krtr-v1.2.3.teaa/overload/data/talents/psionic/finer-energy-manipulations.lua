...
newTalent{
	...
	action = function(self, t)
		local d d = self:showInventory("어느 보석을 사용합니까?", self:getInven("INVEN"), function(gem) return gem.type == "gem" and gem.material_level and not gem.unique end, function(gem, gem_item)
			...
			self.changed = true
			d.used_talent = true
			self:setEffect(self.EFF_CRYSTAL_BUFF, dur, {name=gem.name, kr_name=gem.kr_name or gem.name, effects=gem.wielder}) --@ 한글이름 추가
		end)
		local co = coroutine.running()
		d.unload = function(self) coroutine.resume(co, self.used_talent) end
		if not coroutine.yield() then return nil end
...
newTalent{
	...
	info = function(self, t)
		...
		마석의 경우, this increases the amount of Psi you can store by +%d.
		보석의 경우, 모든 능력치가 %d 만큼 더 상승합니다.]]): --@@ 한글화 필요 #246
