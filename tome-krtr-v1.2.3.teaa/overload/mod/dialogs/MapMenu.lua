...
function _M:generateList()
	...
	if a and config.settings.cheat then list[#list+1] = {name="Lua inspect [Actor]", action="debug-inspect", color=colors.simple(colors.LIGHT_BLUE), actor=a} end --@ 디버그 모드 전용 : 한글화 제외
	if g and config.settings.cheat then list[#list+1] = {name="Lua inspect [Terrain]", action="debug-inspect", color=colors.simple(colors.LIGHT_BLUE), actor=g} end --@ 디버그 모드 전용 : 한글화 제외
	...
	if game.zone and not game.zone.wilderness then
		...
		for tid, _ in pairs(player.talents) do
			...
			if t_avail then
				...
				local tn = t.kr_name or t.name --@ 두줄뒤, 네줄뒤 사용 : 기술 한글이름 저장 
